...
export function PackageManager() {
  ...
  const availableApps = useMemo(() => (
    useCases.getPackagesForPlatformUseCase.executeSync({ platform: os }).packages
  ), [os, useCases]);
  ...
  const categories = packageCatalog.categories;

  // Count per category for the filter chips
  const categoryCounts = packageCatalog.categoryCounts;
...
function PackageCard({
  ...
  useCases: ReturnType<typeof useClientUseCases>;
  isFocused?: boolean;
  onFocus?: () => void;
  tabIndex?: number;
}) {
